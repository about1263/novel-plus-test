...
class TestAuthorBasic:
    ...
    @allure.title("作者基础功能数据驱动测试")
    @pytest.mark.author
    @pytest.mark.parametrize("test_case", load_test_cases_for_parametrize(
        get_test_data_path('author', 'author_basic.yaml')
    ), ids=lambda x: x[0])
    def test_author_basic_cases(self, api_client, authenticated_client, test_case):
        """作者基础功能数据驱动测试"""
...
@allure.feature("作者模块")
@allure.story("作者小说管理")
class TestAuthorBookManagement:
    """作者小说管理测试类"""

    @allure.title("作者小说管理数据驱动测试")
    @pytest.mark.author
    @pytest.mark.parametrize("test_case", load_test_cases_for_parametrize(
        get_test_data_path('author', 'author_book_management.yaml')
    ), ids=lambda x: x[0])
    def test_author_book_management_cases(self, api_client, authenticated_client, test_case):
        """作者小说管理数据驱动测试"""
        case_name, case_data = test_case
...
@allure.feature("作者模块")
@allure.story("作者章节管理")
class TestAuthorChapterManagement:
    """作者章节管理测试类"""

    @allure.title("作者章节管理数据驱动测试")
    @pytest.mark.author
    @pytest.mark.parametrize("test_case", load_test_cases_for_parametrize(
        get_test_data_path('author', 'author_chapter_management.yaml')
    ), ids=lambda x: x[0])
    def test_author_chapter_management_cases(self, api_client, authenticated_client, test_case):
        """作者章节管理数据驱动测试"""
        case_name, case_data = test_case
...
@allure.feature("作者模块")
@allure.story("作者收入统计")
class TestAuthorIncome:
    """作者收入统计测试类"""

    @allure.title("作者收入统计数据驱动测试")
    @pytest.mark.author
    @pytest.mark.parametrize("test_case", load_test_cases_for_parametrize(
        get_test_data_path('author', 'author_income.yaml')
    ), ids=lambda x: x[0])
    def test_author_income_cases(self, api_client, authenticated_client, test_case):
        """作者收入统计数据驱动测试"""
        case_name, case_data = test_case
...
class TestAuthorAI:
    ...
    @allure.title("AI写作助手数据驱动测试")
    @pytest.mark.author
    @pytest.mark.parametrize("test_case", load_test_cases_for_parametrize(
        get_test_data_path('author', 'author_ai.yaml')
    ), ids=lambda x: x[0])
    def test_author_ai_cases(self, api_client, authenticated_client, test_case):
        """AI写作助手数据驱动测试"""
        case_name, case_data = test_case
