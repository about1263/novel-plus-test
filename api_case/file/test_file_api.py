...
def execute_file_test_case(client, test_case):
    """
    执行文件模块测试用例（特殊处理图片响应）
    ...
    # 处理图片验证码接口
    if expected.get('response_type') == 'image':
        try:
            # 使用session直接获取流式响应
            ...
            if content_type_contains:
                ...
                for expected_str in content_type_contains:
                    if expected_str in content_type:
                        ...
                        break
                if not found:
                    log.warning(f"Content-Type检查失败: {content_type} 不包含 {content_type_contains}")
                else:
            ...
            log.warning(f"验证码接口测试异常（可能正常）: {e}")
            # 对于文件流接口，可能需要特殊处理，这里不严格断言
    else:
        # 默认使用通用的execute_test_case
        execute_test_case(client, test_case)


@allure.feature("文件模块")
@allure.story("验证码获取")
class TestFileVerify:
    """验证码获取测试类"""

    @allure.title("文件模块数据驱动测试")
    @pytest.mark.file
    @pytest.mark.parametrize("test_case", load_test_cases_for_parametrize(
        get_test_data_path('file', 'file_verify.yaml')
    ), ids=lambda x: x[0])
    def test_file_verify_cases(self, api_client, authenticated_client, test_case):
        """文件模块数据驱动测试"""
        ...
        # 执行测试用例（使用特殊的文件测试执行函数）
        execute_file_test_case(client, case_data)