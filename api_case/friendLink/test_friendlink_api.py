...
class TestFriendLink:
    ...
    @allure.title("友链模块数据驱动测试")
    @pytest.mark.friendLink
    @pytest.mark.parametrize("test_case", load_test_cases_for_parametrize(
        get_test_data_path('friendLink', 'friendlink.yaml')
    ), ids=lambda x: x[0])
    def test_friendlink_cases(self, api_client, authenticated_client, test_case):
        """友链模块数据驱动测试"""
