...
@allure.feature("用户登录注册模块")
class TestUserLoginModule:
    """用户登录注册模块测试类 - 包含USER_API_01到USER_API_10所有测试用例"""
    
    # 类级别加载测试用例
    yaml_file = get_test_data_path('user_login', 'user_login_module.yaml')
    all_cases = load_test_cases(yaml_file)
    
    # 过滤登录测试用例
    login_cases = [(case['name'], case) for case in all_cases if 'login' in case.get('tags', [])]
    
    # 过滤注册测试用例
    register_cases = [(case['name'], case) for case in all_cases if 'register' in case.get('tags', [])]
    
    @allure.story("用户登录功能测试")
    @allure.title("用户登录数据驱动测试")
    @pytest.mark.user
    @pytest.mark.parametrize("test_case", login_cases, ids=lambda x: x[0])
    def test_user_login_cases(self, api_client, authenticated_client, test_case):
        """用户登录数据驱动测试 - 覆盖USER_API_01到USER_API_05"""
    ...
    @allure.story("用户注册功能测试")
    @allure.title("用户注册数据驱动测试")
    @pytest.mark.user
    @pytest.mark.parametrize("test_case", register_cases, ids=lambda x: x[0])
    def test_user_register_cases(self, api_client, authenticated_client, test_case):
        """用户注册数据驱动测试 - 覆盖USER_API_06到USER_API_10"""
        case_name, case_data = test_case
